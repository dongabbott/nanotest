...
- **AI 智能分析**：支持火山引擎豆包（Doubao）和 OpenAI，自动截图分析和异常检测
...
└─────────────────────────────────────────────────────────────┘
        ...
        ▼                     ▼                     ▼
┌───────────────┐    ┌───────────────┐    ┌───────────────┐
│ Appium Server │    │ Doubao/OpenAI │    │   移动设备     │
│   (移动端)     │    │    (AI/LLM)   │    │  (Android/iOS) │
└───────────────┘    └───────────────┘    └───────────────┘
```

> **开发模式说明**：本地开发默认使用 **SQLite**（`nanotest.db` 文件），无需安装 PostgreSQL。后端会自动检测数据库类型并初始化表结构和默认账户。生产环境建议切换为 PostgreSQL。
...
```env
# 开发模式默认使用 SQLite，无需配置数据库
# 若要使用 PostgreSQL，取消注释并填写：
...
> **SQLite 模式**：首次启动时，后端会自动创建数据库表并生成默认开发账户。
...
前端默认运行在 `http://localhost:5173`，并代理 API 请求到 `http://localhost:8000`。
...
| `ARK_MODEL` | Ark 模型名 | doubao-seed-2-0-pro-260215 |
| `OPENAI_API_KEY` | OpenAI API Key | - |
| `OPENAI_MODEL` | OpenAI 模型 | gpt-4-vision-preview |
| `LLM_BASE_URL` | 自定义 OpenAI 兼容 base_url | - |
| `AI_ANALYSIS_TIMEOUT` | AI 分析超时（秒） | 60 |

> **LLM 优先级**：如果同时配置了 Ark 和 OpenAI，Ark 配置会映射为 OpenAI 兼容格式使用。即 `ARK_API_KEY` → `openai_api_key`，`ARK_BASE_URL` → `llm_base_url`，`ARK_MODEL` → `openai_model`。
